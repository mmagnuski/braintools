...
%  to speed while preserving its functionality - let me know!)
%
% ===================
%   MANDATORY INPUT
% ===================
...
%                    default: true
%    'DiscreteTime'- discrete time, if set to true time ticks are
%                    forbidden not to directly point to the center
%                    of some pixel. Otherwise, they are free to point
%                    anywhere along time axis, even if it is between
%                    two pixels
%
...
%                    if set to false - they will retain their true value
%                    default: false
%     'AddHz'      - if set to true, each tick label in frequency dimension
%                    will have 'Hz' added at the end that is instead of
%                    '10' it will be '10 Hz'
%
...
% FIX:
%      [ ] deal with time if it is given in seconds (ie.
%          -0.5, 0.23 etc.)
...
%          segments... (hggroup?)
%      [ ] plot lines as longer segments, not 1-pixel length only
%      [ ] zero-line not plotted correctly
% CHECK:
%      [ ] change figure behavior!
%      [ ] thorough check of maskitsweet different configurations
%          figure behavior
%      [ ] opens a new figure
...
% CONSIDER:
%      [ ] make maskitsweet an object ?
%      [ ] clear up keys that do not need a following
%          value - how should it behave?
%      [ ] smooth lines? (a'la contour ?)
%      [ ] adding nonlinear FreqJump option (?)
%      [ ] adding Freq ticks at specific user-defined places
...
opt.ZeroLine = true;
opt.DiscreteTime = false;
opt.DiscreteFreq = true;
...
if nargin > 2
    ...
    if current_vargin > 0
        ...
        if ~isempty(adr)
            ...
            handles.figure = get(handles.axis, 'Parent');
            % current_vargin = current_vargin - 2;
        end
    end
...
if ~islogical(mask) && ~isempty(mask)
...
if ~isfield(handles, 'axis') || isempty(handles.axis)
...
% get x and y units vectors:
uni = {'x', 'y'};
fld = {'Time', 'Freq'};
dim = [2, 1];
for i = 1:2
    if ~isempty(opt.(fld{i}))
        units.(uni{i}) = opt.(fld{i})';
    else
        units.(uni{i}) = [1:size(matri, dim(i))]'; %#ok<NBRAK>
    end
...
for l = 1:length(opt.lines)
    if opt.lines(l)
        ...
        xval = units.x(indj);
        rowX = [xval + units.halfx, ...
                xval + units.halfx];

        yval = units.y(indi);
        rowY = [yval - units.halfy, ...
                yval + units.halfy];
        % ====
        ...
        xval = units.x(indj);
        colX = [xval - units.halfx, ...
                xval + units.halfx];
        yval = units.y(indi);
        colY = [yval + units.halfy, ...
                yval + units.halfy];
        % ====
...
if ~isempty(opt.Time)
    ...
    if len ~= size(matri, 2)
        ...
        % CHANGE temporary hack - do not use JumpTime etc.
        %        if DiscreteTime is false
        ...
        if opt.DiscreteTime
            ...
            breakpoint = labind(breakpoint); %#ok<FNDSB>
            ...
            if  opt.ZeroLine
                ...
                if opt.DiscreteFreq
                    yvals = [0.5, size(matri,1)+0.5];
                else
                    yvals = [opt.Freq(1) - halfdif, ...
                        opt.Freq(end) + halfdif];
                end
                
                handles.ZeroLine = line([0, 0], ...
                    yvals, 'Color', [0 0 0], ...
                    'LineWidth', 2, 'Parent',...
                    handles.axis);
            end
    ...
    % set 'universal' features of ticks
    set(handles.axis, 'TickLength', [0.02 0.02], ...
...
if opt.PlotScale
    % CHANGE
    % scale plotting should be updated and work
    ...
    if ~opt.NoRGB
        ...
        if (matv.y > 2014) || (matv.y == 2014 && strcmp(matv.l, 'b'))
            ...
            set(handles.colorbar, 'YTick', pos, 'YTickLabel',...
                ...
                'TickDir', 'out', 'Layer', 'top', 'YLim', ...
                [1 outputs.cmaplen], 'Box', 'off');
...
% JumpLabs finds time or frequency points to label
% when given TickInterval (JumpTime, JumpFreq) is
% given

% look for minima in division by Jump:
timj = Vec/Jump;
timjr = round(timj); % <- CHECK this may not work if Time is in 0.3 seconds
timf = abs(timj - timjr);
timfit = (diff(timf) > 0);
timfit = diff(timfit);
...
% this is a simple input check function
...
% MappingModule takes care of mapping matrix value
% to the colormap
